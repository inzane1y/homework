...
def graph(n_list, n_experiments, n_attempts):
    for n in n_list:
        if not os.path.isfile(f'{n}x{n}x{n}_{n_experiments}_{n_attempts}.txt'):
            pl.percolation(n, n_experiments, n_attempts)

    fig, ax = plt.subplots() 
    for n in n_list:
        ax.plot(
            np.genfromtxt(f'{n}x{n}x{n}_{n_experiments}_{n_attempts}.txt', usecols=0),
            np.genfromtxt(f'{n}x{n}x{n}_{n_experiments}_{n_attempts}.txt', usecols=1),
            label=f'{n}'
        )
