...
def matrix_is_passable(n, matrix):
    ...
    for k in range(n):
        for i in range(n):
            if flag:
                return True
            matrix_recursive_check(n, matrix, i, 0, k)

    return False

# Used in function above
def matrix_recursive_check(n, matrix, i, j, k):
    '''
    Recursively check the path from a given point (i, j, k).
    '''
    # Flag
    global flag
    # Check if current point is false or the path is already found
    if not matrix[i, j, k] or flag:
        return
    ...
    # Find path
    if matrix[i, j + 1, k]:
        matrix_recursive_check(n, matrix, i, j + 1, k)
    if i != 0 and matrix[i - 1, j, k]:
        matrix_recursive_check(n, matrix, i - 1, j, k)
    if i != n - 1 and matrix[i + 1, j, k]:
        matrix_recursive_check(n, matrix, i + 1, j, k)
    if k != 0 and matrix[i, j, k - 1]:
        matrix_recursive_check(n, matrix, i, j, k - 1)
    if k != n - 1 and matrix[i, j, k + 1]:
        matrix_recursive_check(n, matrix, i, j, k + 1)
    if j != 0 and matrix[i, j - 1, k]:
        matrix_recursive_check(n, matrix, i, j - 1, k)
...
def percolation(n, n_experiments, n_attempts):
    ...
    with open(f'{n}x{n}x{n}_{n_experiments}_{n_attempts}.txt', 'w+') as output_file:
